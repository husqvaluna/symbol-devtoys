...
export function meta() {
  return [
    { title: "Namespace - Symbol DevToys" },
    { name: "description", content: "Query the network by namespace ID to retrieve namespace information." },
  ];
}
  ...
  return (
    <SidebarInset>
      ...
      <div className="p-2">
        <Card>
          <CardHeader>
            <CardTitle>Retrieve Namespace Information</CardTitle>
            <CardDescription>Retrieve namespace information from the {selectedNetwork} network</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="namespace-result">Namespace Information (JSON)</Label>
              <Textarea
                id="namespace-result"
                value={JSON.stringify(fetcher.data?.result, null, 2)}
                readOnly
                className="min-h-[300px] font-mono text-sm bg-gray-50 dark:bg-gray-800"
                placeholder="Namespace information will be displayed here"
              />
            </div>
          </CardContent>
        </Card>
